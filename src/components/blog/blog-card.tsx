...
const BlogCard = ({ post }: { post: Post }) => {
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="w-full md:w-3/4 lg:w-4/6 bg-subtleAppBg rounded-sm transition border border-bdr-idle shadow-sm hover:border-bdr-hover hover:shadow outline-none focus:ring focus:ring-appBorder group"
    >
